...
await sql`DELETE FROM "FinancialItem"`;
await sql`DELETE FROM "LegalItem"`;
...
for (const [title, status, priority, notes, dueDate, nextAction] of projects) {
  const id = cuid();
  const ts = now();
  await sql`INSERT INTO "Project" (id, title, status, priority, notes, "dueDate", "nextAction", "createdAt", "updatedAt")
    VALUES (${id}, ${title}, ${status}, ${priority}, ${notes}, ${dueDate}, ${nextAction}, ${ts}, ${ts})`;
}
...
for (const [title, type, status, deadline, notes, documentRef] of legalItems) {
  const id = cuid();
  const ts = now();
  await sql`INSERT INTO "LegalItem" (id, title, type, status, deadline, notes, "documentRef", "createdAt", "updatedAt")
    VALUES (${id}, ${title}, ${type}, ${status}, ${deadline}, ${notes}, ${documentRef}, ${ts}, ${ts})`;
}
...
for (const [title, category, status, amount, frequency, notes, dueDate] of financialItems) {
  const id = cuid();
  const ts = now();
  await sql`INSERT INTO "FinancialItem" (id, title, category, status, amount, frequency, notes, "dueDate", "createdAt", "updatedAt")
    VALUES (${id}, ${title}, ${category}, ${status}, ${amount}, ${frequency}, ${notes}, ${dueDate}, ${ts}, ${ts})`;
}
...
for (const [title, dateTime, endTime, location, status, notes] of bookings) {
  const id = cuid();
  const ts = now();
  await sql`INSERT INTO "Booking" (id, title, "dateTime", "endTime", location, status, notes, "createdAt", "updatedAt")
    VALUES (${id}, ${title}, ${dateTime}, ${endTime}, ${location}, ${status}, ${notes}, ${ts}, ${ts})`;
}
